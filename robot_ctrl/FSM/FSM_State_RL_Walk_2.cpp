...
void FSM_State_RL_Walk_2::run_state() {
    ...
    desired_vel_xyw_command << fsm_data_->rc_->rc_control_.v_des[0]*1.0, fsm_data_->rc_->rc_control_.v_des[1]*1.0, fsm_data_->rc_->rc_control_.v_des[2]*1.0;
    ...
    for (auto &leg: this->fsm_data_->leg_controller_->leg_command) {
        leg.kp_joint = Vec3<double>(25, 25, 25).asDiagonal();
        leg.kd_joint = Vec3<double>(3, 3, 3).asDiagonal();
