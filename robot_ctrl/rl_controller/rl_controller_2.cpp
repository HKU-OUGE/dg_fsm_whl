...
bool RLController2::init() {
    ...
    initialized_ = true;
    running_ = false;
    loadPolicy("../models/policy_aug_15_21_43_HKU.onnx");
...
bool RLController2::step(Vec23<double>* joint_q, Vec22<double>* joint_qd, Vec3<double>* accel, Vec3<double>* desired_vel_xyw, int* stand_flag) {
    ...
    if (step_counter % 10 == 0) {
        ...
        Vec3<double> body_lin_vel;
        body_lin_vel[0] = (*joint_qd)[0];
        body_lin_vel[1] = (*joint_qd)[1]; 
        body_lin_vel[2] = (*joint_qd)[2];
        ...
        // whl velocities
        // reordered_vels.segment<3>(0) = joint_qd->segment<3>(10);
        // reordered_vels.segment<3>(3) = joint_qd->segment<3>(18);
        // reordered_vels.segment<3>(6) = joint_qd->segment<3>(6);
        // reordered_vels.segment<3>(9) = joint_qd->segment<3>(14);
        ...
        // 0. Base linear valocity (3)
        // 1. Base angular velocity (3)
        // 2. Projected gravity vector (3) 
        // 3. Velocity commands (3)
        // 4. Joint angle difference from default pose (12) {LF, LH, RF, RH} without wheel joints
        // 5. Joint velocity (16)  {LF, LH, RF, RH} with wheel joints
        // 6. Last actions taken (16) {LF, LH, RF, RH} position for legged_joints and velocity for wheeled joints

        // observation.segment<3>(0) = body_lin_vel * POLICY_OBS_SCALES ["base_lin_vel"];
        observation.segment<3>(0) = body_ang_vel * POLICY_OBS_SCALES ["base_ang_vel"];
        observation.segment<3>(3) = projected_gravity;
        observation.segment<3>(6) = vel_commands * POLICY_OBS_SCALES ["velocity_commands"];
        // observation[9] = *stand_flag;
        observation.segment<12>(9) = (reordered_angles - default_dof_pos_obs_reorder_) * POLICY_OBS_SCALES ["joint_pos"];
        observation.segment<4>(21) = reordered_vels * POLICY_OBS_SCALES ["joint_vel"];
        // observation.segment<16>(21) = reordered_vels * POLICY_OBS_SCALES ["joint_vel"];
        observation.segment<16>(25) = last_action;
        // std::cout << "observation shape: " << observation.size() << std::endl;
        ...
        // Current observation (dof_obs_ values)
        ...
        for (int i = 0; i < 16; i++) {
            if (i == 12 || i == 13 || i == 14 || i == 15) {
                scaled_actions[i] = output_data[i] * 5.0 + 0.0; // Assuming action_scale is 0.5, adjust if needed
            }
            else
            {
                scaled_actions[i] = output_data[i] * 0.25 + default_dof_pos_reorder[i]; // Assuming action_scale is 0.5, adjust if needed
            }
        }

        // tweak order:
        std::cout << "scaled_actions: " << scaled_actions.segment<4>(12).transpose() << std::endl;
