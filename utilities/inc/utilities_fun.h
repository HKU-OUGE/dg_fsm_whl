...
#ifndef MY_MUJOCO_SIMULATOR_UTILITIES_FUN_H
#define MY_MUJOCO_SIMULATOR_UTILITIES_FUN_H

#include<string>
#include <cmath>
#include "../types/hardware_types.h"

std::string getLcmUrl(int64_t ttl);

template<typename T>
T deadzone_func(T data, T deadzone_width) {
    return (std::abs(data) > deadzone_width) ? data : 0;
}
 ...
 */
    ...
    /**
     * @warning this function return the transpose of the R matrix.
     * @tparam T
     * @param q
     * @return
     */
...
#endif //MY_MUJOCO_SIMULATOR_UTILITIES_FUN_H
